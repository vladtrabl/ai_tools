...
**The chat is the orchestrator.** There is no manager subagent — the chat reads this skill, selects a workflow, and invokes specialist agents directly via the Task tool.
...
| `reviewer` | Review code for quality, correctness, architectural compliance |
| `tester` | Run tests or trace behavior, group failures by root cause |
...
4. If all file paths are disjoint AND all `cascading: false` → dispatch **ALL Fixers simultaneously**
5. Announce before dispatching: *"Launching N fixers in parallel for RC#1, RC#2, ..."*
6. Wait for ALL Fixers to complete before routing to final Tester
...
When all root causes are independent (`all_independent: true`, disjoint locations) — dispatch all Fixers simultaneously, then run one final Tester:
...
| Tester | any | all `cascading: false`, disjoint locations | Dispatch ALL Fixers simultaneously |
...
2.5. **Explore pre-phase (lightweight, orchestrator-only):** When the project root is unfamiliar or no relevant file paths have been provided, run 2–3 file/search calls yourself before invoking the first agent. Maximum 3 searches. Skip when the user has provided sufficient context.
3. **Invoke the first specialist** via the Task tool with a focused prompt.
